...
export interface GenerateQuotePDFArgs {
  quote: QuoteWithRelations
  items: QuoteItem[]
  profile: Profile | null
}

export function generateQuotePDF({ quote, items, profile }: GenerateQuotePDFArgs): void {
  const doc = new jsPDF({ unit: 'mm', format: 'a4' })
  const accent = hexToRgb(profile?.invoice_accent_color || '#E11D48')
  const pageW = 210
  const margin = 20
  ...
  doc.setTextColor(120)
  doc.setFont('helvetica', 'normal')
  doc.text(quote.number, margin, y + 6)
  ...
  doc.text('ISSUE DATE', pageW - margin - 50, y)
  doc.text('VALID UNTIL', pageW - margin, y, { align: 'right' })

  y += 5
  doc.setFontSize(11)
  doc.setTextColor(20)
  doc.setFont('helvetica', 'bold')
  doc.text(quote.clients?.name || '—', margin, y)
  ...
  if (quote.clients?.company) {
    doc.text(quote.clients.company, margin, billY)
    billY += 4
  }
  if (quote.clients?.email) {
    doc.text(quote.clients.email, margin, billY)
    billY += 4
  }
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  doc.text(fmtDate(quote.issue_date), pageW - margin - 50, y)
  doc.text(fmtDate(quote.valid_until), pageW - margin, y, { align: 'right' })
  ...
  doc.text('Subtotal', totalsX, y)
  doc.setTextColor(30)
  doc.text(fmt(quote.subtotal), pageW - margin, y, { align: 'right' })
  y += 5

  doc.setTextColor(100)
  doc.text(`Tax (${quote.tax_rate}%)`, totalsX, y)
  doc.setTextColor(30)
  doc.text(fmt(quote.tax_amount), pageW - margin, y, { align: 'right' })
  y += 6
  ...
  doc.setTextColor(accent[0], accent[1], accent[2])
  doc.setFontSize(14)
  doc.text(fmt(quote.total), pageW - margin, y, { align: 'right' })
