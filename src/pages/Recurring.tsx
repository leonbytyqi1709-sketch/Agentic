...
interface RecurringItemDraft {
  description: string
  quantity: number | string
  unit_price: number | string
}

const EMPTY_ITEM: RecurringItemDraft = { description: '', quantity: 1, unit_price: 0 }
...
export default function Recurring() {
  ...
  const [form, setForm] = useState<RecurringFormState>(EMPTY)
  const [items, setItems] = useState<RecurringItemDraft[]>([{ ...EMPTY_ITEM }])
  const [saving, setSaving] = useState<boolean>(false)
  const [deleteTarget, setDeleteTarget] = useState<RecurringInvoiceWithRelations | null>(null)
  ...
  function openEdit(r: RecurringInvoiceWithRelations) {
    setEditing(r)
    setForm({
      name: r.name || '',
  ...
  async function handleSave(e: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true)
    try {
      const normalizedItems: RecurringInvoiceItem[] = items.map((it) => ({
        description: it.description,
        quantity: Number(it.quantity || 0),
        unit_price: Number(it.unit_price || 0),
      }))
      const payload: RecurringInvoiceInsert = {
        name: form.name,
  ...
  async function toggle(r: RecurringInvoiceWithRelations) {
    try {
  ...
  function updateItem(idx: number, patch: Partial<RecurringItemDraft>) {
    ...
    <AppLayout title="Recurring Invoices">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Set up automatic billing once and watch invoices generate themselves every month."
          action={
