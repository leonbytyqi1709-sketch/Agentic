...
export default function Dashboard() {
  ...
  const inRange = (iso) => !rangeStart || (iso && new Date(iso) >= rangeStart)

  const upcoming = useMemo(() => {
    const now = new Date()
    const in14 = new Date(now.getTime() + 14 * 86400000)
    return projects
      .filter((p) => p.due_date && new Date(p.due_date) >= now && new Date(p.due_date) <= in14)
  ...
  const trendData = useMemo(() => {
    ...
    clients.forEach((c) => c.created_at && (findBucket(c.created_at)?.clients !== undefined) && (findBucket(c.created_at).clients += 1))
    projects.forEach((p) => p.created_at && findBucket(p.created_at) && (findBucket(p.created_at).projects += 1))
    invoices
  ...
  const statusData = useMemo(() => {
    const buckets = { planning: 0, active: 0, on_hold: 0, completed: 0 }
    projects.forEach((p) => {
      if (buckets[p.status] != null) buckets[p.status] += 1
    })
                ...
                {statusData.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      />
                      <Legend iconType="circle" wrapperStyle={{ fontSize: 11, color: 'rgba(250,250,250,0.6)' }} />
                    </PieChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <div className="bg-surface-2 rounded-xl border border-white/[0.06] shadow-card p-5">
              ...
              {activities.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {activities.map((a) => {
                    const meta = ACTIVITY_META[a.type] || { icon: CheckCircle2, color: 'text-text/50', label: a.type }
