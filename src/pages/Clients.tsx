...
import type {
  ClientFilterValue as ClientFilterValueType,
  SavedFilter,
} from '../store/filterStore.js'
...
export default function Clients() {
  ...
  function applyFilter(f: SavedFilter<ClientFilterValueType>) {
    setSearch(f.filter.search || '')
    setStatusFilter((f.filter.statusFilter as StatusFilter) || 'all')
  }

  const filterTabs: StatusFilter[] = ['all', 'active', 'lead', 'inactive']
  ...
  return (
    <AppLayout title="Clients">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-base font-bold tracking-tight text-text truncate">{client.name}</h3>
                    <span
                      className={cn(
                  ...
                  <div className="text-sm text-text/50 truncate">{client.company || '—'}</div>
                  {client.email && (
                    <div className="text-xs text-text/40 truncate mt-0.5">{client.email}</div>
                  )}
                  <div className="mt-2">
      ...
      {/* Create/Edit Modal */}
      <Modal open={modalOpen} onClose={closeModal} title={editing ? 'Edit Client' : 'New Client'}>
