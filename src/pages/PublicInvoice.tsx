...
import type { PublicInvoice as PublicInvoiceType, PublicInvoiceItem } from '../types'
...
export default function PublicInvoice() {
  ...
  const [invoice, setInvoice] = useState<PublicInvoiceType | null>(null)
  const [items, setItems] = useState<PublicInvoiceItem[]>([])
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (e2) throw e2
        const invList = inv as PublicInvoiceType[] | null
        if (!invList?.length) {
          setError('Invoice not found')
        } else {
          setInvoice(invList[0])
          setItems((it as PublicInvoiceItem[] | null) || [])
        }
      } catch (err) {
