...
import { TagBadges } from '../components/TagPicker.js'
import type { Client, ProjectStatus, ProjectWithClient } from '../types'
...
type TabId = 'overview' | 'tasks' | 'time'
...
interface ProjectDetailData extends ProjectWithClient {
  clients?: (Pick<Client, 'name' | 'company'> & { id: string }) | null
}
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data } = await supabase
        ...
        .single()
      if (cancelled) return
      setProject((data as ProjectDetailData | null) ?? null)
      setLoading(false)
    }
    load()
    return () => {
            ...
            {TABS.map(({ id: tabId, label, icon: Icon }) => (
              <button
                ...
                className={cn(
                  ...
                  tab === tabId ? 'bg-primary/10 text-primary' : 'text-text/50 hover:text-text'
                )}
              >
